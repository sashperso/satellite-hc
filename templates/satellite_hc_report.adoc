...
Pulp3 is {% if pulp3_absent == false %}installed on this Satellite Server.{% else %}absent from this Satellite Server.

{% if pulp3_absent == true %}
...
// UNCOMMENT ME! {{ result_errata.stdout_lines | to_nice_yaml }}
...
{{ result_sync_plan.stdout_lines | to_nice_yaml }}
...
{{ result_activation_key.stdout_lines | to_nice_yaml }}
